...
import { registerWorkflowNavigator, manualDeletionPlan, type WorkflowNavigatorDependencies } from "../src/host-navigator.js";
...
void test("navigator refuses deletion of a run needed by a surviving retry", async () => {
  ...
  const child = new RunStore(cwd, "session", "retry-child", home);
  await child.create({ id: child.runId, workflowName: "retry", cwd, sessionId: "session", state: "interrupted", parentRunId: source.runId, retry: { sourceRunId: source.runId, lineageRootRunId: source.runId, completedPaths: ["agent/replayable"], incompletePaths: [], namedWorktrees: [] }, agents: [], agentSessions: [] }, snapshot);
  const commands: Array<{ handler: (args: string, ctx: unknown) => Promise<void> }> = [];
  ...
  assert.ok(command);
  await executeCommand(command, "", ctx);
  assert.ok(notifications.some((message) => message.includes(child.runId) && message.includes("depends on")));
...
void test("navigator bulk deletion protects dependencies of surviving runs", async () => {
  ...
  await source.create({ id: source.runId, workflowName: "source", cwd, sessionId: "session", state: "failed", agents: [], agentSessions: [] }, snapshot);
  await source.complete("agent/replayable", "kept");
  const child = new RunStore(cwd, "session", "interrupted-child", home);
  await child.create({ id: child.runId, workflowName: "retry", cwd, sessionId: "session", state: "interrupted", parentRunId: source.runId, retry: { sourceRunId: source.runId, lineageRootRunId: source.runId, completedPaths: ["agent/replayable"], incompletePaths: [], namedWorktrees: [] }, agents: [], agentSessions: [] }, snapshot);
  const commands: Array<{ handler: (args: string, ctx: unknown) => Promise<void> }> = [];
  const notifications: string[] = [];
  const pi = { registerTool() {}, registerCommand(_name: string, options: (typeof commands)[number]) { commands.push(options); }, on() {}, getThinkingLevel: () => "medium" as const, getActiveTools: () => [] };
  workflowExtension(testExtensionApi(pi), home);
  let selectCall = 0;
  const ctx = { cwd, hasUI: true, sessionManager: { getSessionId: () => "session" }, ui: { notify(message: string) { notifications.push(message); }, select: async () => { selectCall += 1; return selectCall === 1 ? "Delete all failed" : "Close"; }, confirm: async () => true } };
  const command = commands[0]?.handler;
  assert.ok(command);
  await executeCommand(command, "", ctx);
  assert.equal(existsSync(source.directory), true);
...
void test("navigator bulk deletion protects borrowed worktrees of surviving retries", async () => {
  ...
  await source.create({ id: source.runId, workflowName: "source", cwd, sessionId: "session", state: "failed", agents: [], agentSessions: [] }, snapshot);
  await source.complete("agent/replayable", "kept");
  const owner = structuralPath("worktree", "named", "reviewer");
  const sourceWorktree = await source.worktree(owner);
  const survivor = new RunStore(cwd, "session", "interrupted-retry", home);
  await survivor.create({ id: survivor.runId, workflowName: "retry", cwd, sessionId: "session", state: "interrupted", parentRunId: source.runId, retry: { sourceRunId: source.runId, lineageRootRunId: source.runId, completedPaths: ["agent/replayable"], incompletePaths: [], namedWorktrees: ["reviewer"] }, agents: [], agentSessions: [] }, snapshot);
  const borrowed = await survivor.worktree(owner);
  assert.equal(borrowed.path, sourceWorktree.path);
  writeFileSync(join(borrowed.path, "survivor-work.txt"), "survivor\n");
  const commands: Array<{ handler: (args: string, ctx: unknown) => Promise<void> }> = [];
  const notifications: string[] = [];
  const pi = { registerTool() {}, registerCommand(_name: string, options: (typeof commands)[number]) { commands.push(options); }, on() {}, getThinkingLevel: () => "medium" as const, getActiveTools: () => [] };
  workflowExtension(testExtensionApi(pi), home);
  ...
  await executeCommand(command, "", ctx);
  assert.equal(existsSync(source.directory), true);
  assert.equal(existsSync(survivor.directory), true);
  assert.equal(existsSync(join(borrowed.path, "survivor-work.txt")), true);
  assert.deepEqual(await source.replay("agent/replayable"), { path: "agent/replayable", value: "kept" });
  assert.ok(notifications.some((message) => message.includes(source.runId) && message.includes("surviving")));
});
void test("manual deletion orders retry children before their failed sources", async () => {
  ...
  await child.create({ id: child.runId, workflowName: "child", cwd, sessionId: "session", state: "failed", parentRunId: source.runId, agents: [], agentSessions: [] }, snapshot);
  const entries = [{ store: source, loaded: await source.load(), resolvedAt: undefined }, { store: child, loaded: await child.load(), resolvedAt: undefined }];
  const plan = manualDeletionPlan(entries, "failed", new Map([[source.runId, []], [child.runId, [source.runId]]]));
  assert.deepEqual(plan.ordered.map(({ store }) => store.runId), [child.runId, source.runId]);
});
